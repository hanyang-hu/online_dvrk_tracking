...
"""
python scripts/custom_data_generation.py --overlay_skeleton  --sample_number 1000 --bag_idx 1 --overlay_mask --t_offset -0.4
python scripts/custom_data_generation.py --overlay_skeleton  --sample_number 1000 --bag_idx 2 --overlay_mask --t_offset -0.25 --target_bag "./data/custom/bag0/"
python scripts/custom_data_generation.py --overlay_skeleton  --sample_number 1000 --bag_idx 3 --overlay_mask --t_offset -0.27 --target_bag "./data/custom/bag1/" --frame_end 200
python scripts/custom_data_generation.py --overlay_skeleton  --sample_number 1000 --bag_idx 4 --overlay_mask --t_offset -0.35 --target_bag "./data/custom/bag2/"
python scripts/custom_data_generation.py --overlay_skeleton  --sample_number 1000 --bag_idx 5 --overlay_mask --t_offset -0.2 --target_bag "./data/custom/bag3/"
python scripts/custom_data_generation.py --overlay_skeleton  --sample_number 1000 --bag_idx 6 --overlay_mask --t_offset -0.3 --target_bag "./data/custom/bag4/" --frame_end 50
python scripts/custom_data_generation.py --overlay_skeleton  --sample_number 1000 --bag_idx 7 --overlay_mask --t_offset -0.2 --target_bag "./data/custom/bag5/" --frame_start 50 --frame_end 100
python scripts/custom_data_generation.py --overlay_skeleton  --sample_number 1000 --bag_idx 8 --overlay_mask --t_offset -0.2 --target_bag "./data/custom/bag6/"
python scripts/custom_data_generation.py --overlay_skeleton  --sample_number 1000 --bag_idx 9 --overlay_mask --t_offset -0.4 --target_bag "./data/custom/bag7/"
python scripts/custom_data_generation.py --overlay_skeleton  --sample_number 1000 --bag_idx 10 --overlay_mask --t_offset -0.4 --target_bag "./data/custom/bag8/" --frame_start 50
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", type=str, default="./data/pf_dataset/", help="Directory containing the data files")
    parser.add_argument("--bag_idx", type=str, default=1, help="Index of the ROS bag file")
    parser.add_argument("--downsample_scale", type=int, default=1, help="Factor by which to downsample the images for display")
    ...
    parser.add_argument("--t_offset", type=float, default=0.0, help="Time offset (in seconds) to apply when synchronizing joint readings with images")
    parser.add_argument("--target_bag", type=str, default=None, help="Path to save the processed data. If None, will not save.")
    ...
    args.bag = f"bag{args.bag_idx}/bag{args.bag_idx}_0.db3"
    ...
    typestore = get_typestore(Stores.ROS2_HUMBLE)  # or FOXY, IRON, etc.
    MAX_DT = 0.02  # max allowed time difference between synced messages
    ...
    #         if topic == "/stereo/left/rectified_downscaled_image":
    #             latest_left = get_rgb(msg)
    #             ts_left = timestamp * 1e-9  # convert to seconds
    #             left_count += 1

    #         elif topic == "/stereo/right/rectified_downscaled_image":
    #             latest_right = get_rgb(msg)
    #             ts_right = timestamp * 1e-9  # convert to seconds
    #             right_count += 1

    #         elif topic == "/PSM1/measured_js":
    #             latest_joint = np.array(msg.position)
    #             ts_joint = timestamp * 1e-9  # convert to seconds
    #             joint_count += 1

    #         elif topic == "/PSM1/jaw/measured_js":
    #             latest_jaw = np.array(msg.position)
    #             ts_jaw = timestamp  * 1e-9  # convert to seconds
    #             jaw_count += 1

    #         # Try to synchronize messages
    #         if all(v is not None for v in [latest_left, latest_right, latest_joint, latest_jaw]):
    #             # Check timestamp differences
    #             times = np.array([ts_left, ts_right, ts_joint, ts_jaw])
    #             max_diff = np.max(times) - np.min(times)

    #             if max_diff <= MAX_DT:
    #                 joint_angle = np.concatenate([latest_joint, latest_jaw])  # shape (7,)
    #                 frame_cnt += 1
    ...
    #                     # print(f"Appended frame {appended_count} | Max dt: {max_diff:.6f}s")
    #             else:
    #                 # print(f"Dropped frame due to max dt {max_diff:.6f}s")
    #                 pass

    # print(f"Finished loading data. Counts -> Left: {left_count}, Right: {right_count}, Joint: {joint_count}, Jaw: {jaw_count}")
    # print(f"Total synchronized frames appended: {appended_count}")

    left_msgs, right_msgs, joint_msgs, jaw_msgs = [], [], [], []
    ...
    with Reader(bagpath) as reader:
        for conn, ts, raw in reader.messages():
            ...
            if conn.topic == "/stereo/left/rectified_downscaled_image":
                left_msgs.append((t, get_rgb(msg)))
            elif conn.topic == "/stereo/right/rectified_downscaled_image":
                right_msgs.append((t, get_rgb(msg)))
            elif conn.topic == "/PSM1/measured_js":
                joint_msgs.append((t, np.array(msg.position)))
            elif conn.topic == "/PSM1/jaw/measured_js":
                jaw_msgs.append((t, np.array(msg.position)))

    # Option 2
    # def find_closest(target_t, messages):
    #     # Binary search could be faster
    #     times = [m[0] for m in messages]
    #     idx = np.argmin(np.abs(np.array(times) - target_t))
    #     return messages[idx]

    # frame_buffer = []
    # for t_left, left_img in left_msgs:
    #     right_t, right_img = find_closest(t_left, right_msgs)
    #     joint_t, joint_pos = find_closest(t_left, joint_msgs)
    #     jaw_t, jaw_pos = find_closest(t_left, jaw_msgs)

    #     times = np.array([t_left, right_t, joint_t, jaw_t])
    #     if np.max(times) - np.min(times) <= MAX_DT:
    #         joint_angle = np.concatenate([joint_pos, jaw_pos])
    #         frame_buffer.append((left_img, right_img, joint_angle))

    # Option 3
    # --- Helper function: linear interpolation ---
    def interpolate(target_t, messages):
        """
        Interpolate messages to the target timestamp.
        Only valid if target_t is within message bounds.
        messages: list of (t, data) sorted by t
        Returns: interpolated data
        """
        times = [m[0] for m in messages]
        data = [m[1] for m in messages]

        # Discard out-of-bounds
        if target_t < times[0] or target_t > times[-1]:
            # print(f"Warning: target_t {target_t:.6f} out of bounds ({times[0]:.6f}, {times[-1]:.6f})")
            return None

        # Find the two nearest timestamps
        idx = np.searchsorted(times, target_t)
        t0, t1 = times[idx-1], times[idx]
        # print(t1-t0, target_t-t0)
        d0, d1 = data[idx-1], data[idx]

        alpha = (target_t - t0) / (t1 - t0)
        return (1 - alpha) * d0 + alpha * d1

    # --- Helper function: closest message ---
    def find_closest(target_t, messages):
        times = [m[0] for m in messages]
        idx = np.argmin(np.abs(np.array(times) - target_t))
        return messages[idx][1]  # just return the data

    # --- Build frame buffer ---
    t_offset = args.t_offset
    frame_buffer = []
    for t_left, left_img in left_msgs:
        # Keep closest right image (no interpolation)
        right_img = find_closest(t_left, right_msgs)

        # Interpolate joint and jaw angles
        joint_pos = interpolate(t_left+t_offset, joint_msgs)
        jaw_pos = interpolate(t_left+t_offset, jaw_msgs)

        # Discard frame if interpolation not possible
        if joint_pos is None or jaw_pos is None:
            continue

        joint_angle = np.concatenate([joint_pos, jaw_pos])
        frame_buffer.append((left_img, right_img, joint_angle))

    print(f"Finished building frame buffer with {len(frame_buffer)} frames after interpolation (skipping the first {args.frame_start} and last {args.frame_end} frames).")

    if args.target_bag is not None:
        os.makedirs(args.target_bag, exist_ok=True)
        
        # Save the joint angles as a yaml file (7 values for each frame 0, ..., len(frame_buffer)-1)
        joint_angles_dict = {f"{i - args.frame_start}": frame_buffer[i][2].tolist() for i in range(args.frame_start, len(frame_buffer) - args.frame_end)}
        with open(os.path.join(args.target_bag, "joint_angles.yaml"), "w") as f:
            yaml.dump(joint_angles_dict, f)

        # Save left and right videos
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out_left = cv2.VideoWriter(os.path.join(args.target_bag, "left.mp4"), fourcc, 30.0, (cam.img_size[0]//args.downsample_scale, cam.img_size[1]//args.downsample_scale))
        out_right = cv2.VideoWriter(os.path.join(args.target_bag, "right.mp4"), fourcc, 30.0, (cam.img_size[0]//args.downsample_scale, cam.img_size[1]//args.downsample_scale))

        cnt = 0

        # Write frames to videos
        for left_img, right_img, _ in frame_buffer:

            if args.frame_start > 0:
                args.frame_start -= 1
                continue

            if args.frame_end > 0 and cnt >= len(frame_buffer) - args.frame_end + args.frame_start:
                break
    
            left_resized = cv2.resize(left_img, (cam.img_size[0]//args.downsample_scale, cam.img_size[1]//args.downsample_scale))
            right_resized = cv2.resize(right_img, (cam.img_size[0]//args.downsample_scale, cam.img_size[1]//args.downsample_scale))
            # Convert RGB to BGR for OpenCV
            left_resized = cv2.cvtColor(left_resized, cv2.COLOR_RGB2BGR)
            right_resized = cv2.cvtColor(right_resized, cv2.COLOR_RGB2BGR)
            out_left.write(left_resized)
            out_right.write(right_resized)
            cnt += 1
        out_left.release()
        out_right.release()

    print(f"Finished building frame buffer with {len(frame_buffer)} frames. Starting processing and visualization...")

    time_lst = []

    for latest_left, latest_right, latest_joint in frame_buffer:

        start_t = time.time()

        # left_img, right_img = cam.processImage(latest_left.copy(), latest_right.copy())
        left_img, right_img = latest_left.copy(), latest_right.copy()
