...
if __name__ == "__main__":
    ...
    # Read joint angle files
    joint_angle_readings = []
    data_dir = os.path.join("./data/surgpose/", args.video_label, args.machine_label)
    ...
    print(f"Loaded joint angles for {len(joint_angle_readings)} frames.")
    ...
    while frame_idx < len(joint_angle_readings):
        ...
        if not init_done:
            ...
            cTr, joint_angles, loss = tracker.track_frame(ref_mask=mask, joint_angles=joint_angle_readings[frame_idx], is_init=True, keypoints=torch.from_numpy(kpts).to(model.device).float() if kpts is not None else None)
            ...
            torch.cuda.synchronize()
            start_time = time.time()
            cTr, joint_angles, loss = tracker.track_frame(ref_mask=mask, joint_angles=joint_angle_readings[frame_idx], is_init=False, keypoints=None)
            torch.cuda.synchronize()
            end_time = time.time()
            track_time_lst.append(end_time - start_time)

        cTr_lst.append(cTr.clone())
        joint_angles_lst.append(joint_angles.clone())
        # print(frame_idx)
        frame_idx += 1
    ...
    cTr_seq = torch.stack(cTr_lst).cpu()
    joint_angles_seq = torch.stack(joint_angles_lst).cpu()
    ...
    data_label= f"surgpose_{args.video_label}_{args.machine_label}"
