...
if __name__ == "__main__":
    ...
    def plot_dimension(ax, ref, cma_w, cma_wo, gd_w, gd_wo, name, is_angle=False):
        ...
        if is_angle:
            if SHOW["cma_w"]:
                curves["CMA-ES (w/ joint, 3 iter/frame)"], ref_proc = unwrap_and_align(cma_w, ref)
            if SHOW["cma_wo"]:
                curves["CMA-ES (w/o joint, 3 iter/frame)"], _ = unwrap_and_align(cma_wo, ref)
            if SHOW["gd_w"]:
            ...
            ref_np = ref.numpy()
            if SHOW["cma_w"]:
                curves["CMA-ES (w/ joint, 3 iter/frame)"] = cma_w.numpy()
            if SHOW["cma_wo"]:
                curves["CMA-ES (w/o joint, 3 iter/frame)"] = cma_wo.numpy()
            if SHOW["gd_w"]:
